...
class PointAnchorLHS_PointAnchorRHSKeyPath_Tests: XCTestCase {
    
    // MARK: - Equal
    
    func test_equal() throws {
        let layout = makeConstraints { views in
            views.detail.anchors.bottom.left == \.subtitleLabel.anchors.top.right
        }
    ...
    func test_equal_modified() throws {
        let layout = makeConstraints { views in
            views.detail.anchors.top.left == \.subtitleLabel.anchors.top.right
                ~ .priority(.defaultLow)
                ~ .inset(by: 80)
                ~ .multiplied(by: 1.5)
        }

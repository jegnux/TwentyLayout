...
class PointAnchorLHS_RHSBaseKeyPath_Tests: XCTestCase {
    
     // MARK: - Equal
         
        func test_equal() throws {
            let layout = makeConstraints { views in
                views.detail.anchors.leading.bottom == \.subtitleLabel
            }
        ...
        func test_equal_modified() throws {
            let layout = makeConstraints { views in
                views.detail.anchors.trailing.firstBaseline == \.subtitleLabel
                    ~ .priority(.defaultLow)
                    ~ .inset(by: 80)
                    ~ .multiplied(by: 1.5)
            }
        ...
        func test_lessThanOrEqual() throws {
            let layout = makeConstraints { views in
                views.detail.anchors.centerX.lastBaseline <= \.subtitleLabel
            }
        ...
        func test_lessThanOrEqual_modified() throws {
            let layout = makeConstraints { views in
                views.detail.anchors.top.trailingMargin <= \.subtitleLabel
                    ~ .priority(.defaultLow)
                    ~ .inset(by: 80)
                    ~ .multiplied(by: 1.5)
            }
    ...
    func test_greaterThanOrEqual() throws {
        let layout = makeConstraints { views in
            views.detail.anchors.topMargin.leadingMargin >= \.subtitleLabel
        }
    ...
    func test_greaterThanOrEqual_modified() throws {
        let layout = makeConstraints { views in
            views.detail.anchors.bottomMargin.centerXWithinMargins >= \.subtitleLabel
                ~ .priority(.defaultLow)
                ~ .inset(by: 80)
                ~ .multiplied(by: 1.5)
        }

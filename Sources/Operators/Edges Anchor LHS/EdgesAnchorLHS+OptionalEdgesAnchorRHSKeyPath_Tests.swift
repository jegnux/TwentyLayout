...
class EdgesAnchorLHS_OptionalEdgesAnchorRHSKeyPath_Tests: XCTestCase {
    
     // MARK: - Equal
         
        func test_equal() throws {
            let layout = makeConstraints { views in
                views.detail.anchors.edges == \.superview?.anchors.margins
            }
        ...
        func test_equal_modified() throws {
            let layout = makeConstraints { views in
                views.detail.anchors.directionalEdges == \.superview?.anchors.directionalMargins
                    ~ .priority(.defaultLow)
                    ~ .offset(by: 80)
                    ~ .multiplied(by: 1.5)
            }

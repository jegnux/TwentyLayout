...
class EdgesAnchorLHS_EdgesAnchorRHSKeyPath_Tests: XCTestCase {
    
     // MARK: - Equal
         
        func test_equal() throws {
            let layout = makeConstraints { views in
                views.detail.anchors.edges == \.subtitleLabel.anchors.margins
            }
        ...
        func test_equal_modified() throws {
            let layout = makeConstraints { views in
                views.detail.anchors.directionalEdges == \.subtitleLabel.anchors.directionalMargins
                    ~ .priority(.defaultLow)
                    ~ .offset(by: 80)
                    ~ .multiplied(by: 1.5)
            }

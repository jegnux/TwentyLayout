...
class PointAnchorLHS_OptionalPointAnchorRHSKeyPath_Tests: XCTestCase {
    
    // MARK: - Equal
    
    func test_equal() throws {
        let layout = makeConstraints { views in
            views.detail.anchors.bottom.left == \.superview?.anchors.top.right
        }
    ...
    func test_equal_modified() throws {
        let layout = makeConstraints { views in
            views.detail.anchors.top.left == \.superview?.anchors.top.right
                ~ .priority(.defaultLow)
                ~ .inset(by: 80)
                ~ .multiplied(by: 1.5)
        }
